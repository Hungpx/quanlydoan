...
class PhanBienMapper extends ModelMapper{
    ...
    public function baocaoDiem($model){
        ...
        if ($model->getTrangthai()){
            $whereArr[] = 'pb.trangthai = "'.$model->getTrangthai().'"';
        }
        if ($model->getMaGV()){
            $whereArr[] = '(pb.maGVPB1 = "'.$model->getMaGV().'" OR pb.maGVPB2 = "'.$model->getMaGV().'" OR  pb.maGVPB3 = "'.$model->getMaGV().'" )';
        }
        ...
        $queryStr = 'select pb.maDoan, pb.maSV, pb.maGVPB1, pb.maGVPB2, pb.maGVPB3, pb.trangthai, pb.diemPB1, pb.diemPB2, pb.diemPB3 ,hd.diem
          FROM '.self::TABLE_NAME.' as pb
          INNER JOIN huongdan as hd
          ON hd.maSV = pb.maSV AND hd.maDoan = pb.maDoan
         ';
